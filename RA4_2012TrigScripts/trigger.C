...
TGraphAsymmErrors* drawEff(TH1F *denom, TH1F *num, TString xtitle, TString title_text, int rebin, float xlow,
			   float xhigh, int icol, int imark, TString draw) {

  cout << "correct method called." << endl;
  ...
  // TH1D *Eff=new TH1D("effi","",ref->GetNbinsX(),ref->GetXaxis()->GetXmin(),ref->GetXaxis()->GetXmax());
  TGraphAsymmErrors *Eff = new TGraphAsymmErrors();
  cout << "is this bayesDivide?" << endl;
  Eff->BayesDivide(denom,num);
  cout << "after bayesdivide" << endl;
  //Eff->Draw(); return;
  ...
  // These params are a start.  See how it fits.
  Double_t params[3] = {1.,250.,20.};            
  fermiFunction->SetParameters(params);
  fermiFunction->SetParNames("#epsilon","#mu","#sigma");
  fermiFunction->SetLineColor(4);
  fermiFunction->SetLineWidth(3.5);
  ...
  Eff->GetYaxis()->SetTitle("Efficiency");
  Eff->GetYaxis()->SetRangeUser(0.,1.05);
  Eff->GetXaxis()->SetRangeUser(xlow,xhigh);
  Eff->SetTitle(title_text);
  //  ps=(TPaveStats*)Eff->GetListOfFunctions().FindObject("stats");
  //  ps->SetX1NDC(0.15);
  //  ps->SetX2NDC(0.40);
  // Eff->PaintFit(fermiFunction);
